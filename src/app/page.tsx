...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // 특정 대사 시 3초간 특수 이미지 연출 (책장 넘김 / 캔커피 / 볼펜 선물 / 두근두근 효과 / 학식 합석 / 물컵 전달 / 요구르트 / 숟가락 낙하 / 새 수저 / 휴지 한 장 / 식당 메뉴 / 식당 퇴장 / 도서관 볼펜)
    const isEp2Choice1 = 
    ...
    if (isEp2Choice1 || isEp2Choice2 || isEp2End || isEp3Intro || isEp3Choice1 || isEp3Choice3 || isEp3SpoonDrop || isEp3NewSpoon || isEp3Tissue || isEp3Menu || isEp3RestaurantOut || isEp4Ballpen) {
      if (timedOverlayTimerRef.current) clearTimeout(timedOverlayTimerRef.current);
    ...
    if (isEp2Choice3) {
      if (heartbeatOverlayTimerRef.current) clearTimeout(heartbeatOverlayTimerRef.current);
    ...
    if (pendingChoice || !isProfessorVoiceEnabled) {
    ...
    <main className="min-h-screen text-black">
      ...
      {phase === "screen4_8_chapter" && currentChapterInfo && storyCursor && (
        <section className="relative min-h-screen overflow-hidden">
          ...
          {/* 선택지 이후 특수 연출 이미지 오버레이 (3초 후 사라짐) */}
          {timedOverlayImage && (
            <div className="pointer-events-none fixed inset-0 z-[110] flex items-center justify-center p-4">
              ...
              <div className="relative animate-in fade-in zoom-in duration-500 ease-out">
                ...
                <img
                  ...
                  alt="Special Event"
                  className="h-auto max-h-[60vh] w-auto max-w-[min(85vw,700px)] object-contain drop-shadow-[0_20px_50px_rgba(0,0,0,0.6)] rounded-[32px] border-[5px] border-black overflow-hidden"
                  draggable={false}
                />
              </div>
            </div>
